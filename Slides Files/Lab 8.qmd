...
Be careful! It may seem that the `poly()` function saves us a lot of time, but it does some interesting stuff behind the scenes to calculate polynomials. Do not interpret the regression coefficients of regressions that use `poly()` without first reading the appendix of this Lab *very carefully*! 
::: 
...
# Appendix: The `poly()` function


## The `poly()` function 

The `poly()` is interesting. Namely, it generates polynomials up to specified degree of the given variable. 


:::: {.columns}
::: {.column width="50%"}

For example if we give a variable `1,2,3`, `poly()` will return:

```{r}
# for a polynomial of 3rd degree (cubic)
data.frame(poly(c(1,2,3), 3, raw = TRUE))
```

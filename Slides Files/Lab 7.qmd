...
::: {.fragment fragment-index=2}
When you want to calculate power for $\Delta R^2$ (see [Lab 6](https://quinix45.github.io/PSYC-7804-Regression-Lab-Slides/Slides%20Files/Lab%206.html#/testing-gain-in-prediction-delta-r2){target="_blank"}), whether a set of predictors *B* provide a significant improvement over an original set of predictors *A*,
...
::: {.fragment fragment-index=3}
Larger $R^2$ value imply larger $f^2$ values. Some guidelines are that $f^2$ of .02, .15, and .35 can be interpreted as small, medium, and large effect sizes respectively. Why do we use $f^2$? Check out the [appendix](#appendix) if you are curious!
:::
...
::: {.fragment fragment-index=3}
<div style="font-size: 24px; padding-bottom: 15px;"> Notice that the `pwr` function will give you the missing function argument. We did not specify `v`, and `pwr` returns it as `61.8227`. </div>
:::
...
::: {.fragment fragment-index=4}
<div style="font-size: 23px"> **The Problem:** Since we know that partial slopes depend on all other variables in the model, to do accurate power analysis for regression coefficients in multiple regression, you also need to to specify the correct correlation among your slope of interest and *all other predictors* 😱. This is a bit impractical. </div>
:::

...
The `ppcor` package [@kimPpcorPackageFast2015] contains functions to calculate partial and semi-partial correlations.
:::
...
We are often (actually always) interested in comparing different models. Usually we ask: is it *worth it* to add more variables as predictors? 

In the case of regression, the most popular way of comparing models is by comparing $R^2$ from nested model. Let's go back to how happiness (`happiness`) relates to age (`age`); then, we want to know whether adding number of friends (`friends`) improves our prediction.
...
The "problem" with most popular measures of model fit such as $R^2$ is that they always increase as variables are added, even though the added variables are unrelated to $Y$. 
...
In hierarchical regression we check whether adding variables to a model yields a significant $R^2$ improvement ($\Delta R^2$). We can use the `anova()` function to compare [nested regression models](https://www.statology.org/nested-model/){target="_blank"}. The *F*-test tells us whether the $R^2$ improvement is significant.
...
<div style="font-size: 23px"> Unsurprisingly, adding adding a random variable does not 
provide a significant improvement in variance explained, $\Delta R^2 = .003, F(1, 96) = 0.81, p = .37$. </div>
...
There also exist a family of statistics used for comparing model based on [information theory](https://en.wikipedia.org/wiki/Information_theory){target="_blank"}. What you will often see are the Akaike information criterion [AIC\; @akaikeNewLookStatistical1974] and Bayesian information criterion [BIC\;  @schwarzEstimatingDimensionModel1978]. 

Both AIC and BIC calculate model fit, and then penalize it based on model complexity (number of variables in regression). The logic is that if you are going to add an extra variable to a regression, it should improve fit enough to offset the penalty term. For regression:
...
We can calculate both AIC and BIC for all our models at once with the `AIC()` and `BIC()` functions. In both cases, the smallest value is the model that fits best according to the criteria. 
...
Finally, information criteria like the AIC and BIC can be used to compare non-nested models, unlike $\Delta R^2$, which can only be used to compare nested models. 
